...
if (session_status() === PHP_SESSION_NONE) { session_name('agro_farmer'); session_start(); }
if (!isset($_SESSION['phonenumber'])) {
    echo "<script>window.open('../auth/FarmerLogin.php','_self')</script>";
    exit;
}
if (isset($_GET['delete']) && isset($_GET['id']) && isset($_SESSION['phonenumber'])) {
    $prodId = intval($_GET['id']);
    $sessPhone = intval($_SESSION['phonenumber']);
    $farmerQuery = "SELECT farmer_id FROM farmerregistration WHERE farmer_phone = $sessPhone";
    if ($farmerRes = mysqli_query($con, $farmerQuery)) {
        if ($farmerRow = mysqli_fetch_assoc($farmerRes)) {
            $farmerId = (int)$farmerRow['farmer_id'];
            $prodQuery = "SELECT product_image, farmer_fk FROM products WHERE product_id = $prodId";
            if ($prodRes = mysqli_query($con, $prodQuery)) {
                if ($prodRow = mysqli_fetch_assoc($prodRes)) {
                    if ((int)$prodRow['farmer_fk'] === $farmerId) {
                        mysqli_query($con, "UPDATE products SET product_stock = 0, product_price = 0, product_delivery = 'no' WHERE product_id = $prodId");
                        echo "<script>alert('Product removed from catalogue');window.open('MyProducts.php?msg=removed','_self');</script>";
                        exit;
                    } else {
                        echo "<script>alert('Unauthorized deletion');window.open('MyProducts.php','_self');</script>";
                        exit;
                    }
                }
            }
                                   ...
                                   <div class='row'>
                                        ...
                                        <div class='col-md-6 blackgoldie'>
                                             ...
                                             <div class='row' style='padding:7px; margin-bottom:15px;' >
                                                  ...
                                                  <div class='col-12 col-xl-4 col-lg-4 col-md-4 col-sm-12 aligncenter'> <a href='FarmerProductDetails.php?id=$prod_id&delete=1' class='btn btn-danger border-secondary' style='color:white' onclick=\"return confirm('Delete this product?');\"><b>Delete Product</b></a></div>
